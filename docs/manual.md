...
左侧底栏设置按钮左边的书本图标打开「操作手册」页签，按钮 28×28 px、图标 18 px，间距 4 px，展开时底栏为 36 px；收起时垂直排列工作区、手册、设置和展开按钮。无需导入影像即可阅读。测量、MPR 分割和 VOI 面板提供相同的帮助入口。
...
- 右侧导出面板和整序列导出对话框只保留「查看导出说明」链接，直接进入导出章节；该章说明格式、匿名范围、测量报告及结果文件定位。测量面板按完成、编辑、复制、历史、取消、删除分条显示简要操作提示。
...
| 分类 | 内容 |
| --- | --- |
| 快速开始 | 拖拽文件 / 压缩包 / 文件夹 / PACS 导入、数据源入口、查询、序列选择、页签、工作区保存／恢复与撤销重做 |
...
| 导出与常见问题 | PNG 实际画面、原始 DICOM、测量 CSV／PDF、取消/失败、禁用原因与未支持功能 |

测量示例来自合成影像，通过真实 QML 绘制四种测量，包含矩形控制点与 ROI 信息卡。ROI 使用模态像素统计，宽高统一开关且与面积同行；显示样式持久化，已完成的测量对象可随工作区文件保存／恢复，并可导出 CSV 或 PDF。操作范围见 [工作区与测量导出](workspaces.md)。
