...
每个 2D 视口左上角可切换 Stack、Axial、Coronal、Sagittal。菜单区分原始切片与标准切面；三个标准切面使用当前序列进行 2D MPR 重建，不会切换到其他源序列。所需数据不完整时会在视口内提示，仍可选择 Stack 返回原始切片。各方向独立保留切片位置、窗值、缩放及测量；布局、各视口的序列和方向随工作区保存恢复。四角信息及毫米位置保持原有设置，隐藏四角信息后仍可使用切换入口。
