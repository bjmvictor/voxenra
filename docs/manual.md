...
包含「支持的影像与格式」与「MR 基础阅片」在内共 36 个章节。详细的实现和文件语义另见 [本地导入](local-import.md)、[工作区与测量结果](workspaces.md)、[导出](export.md)。手册只说明已经支持的能力。
...
MPR 右侧「MPR 布局」提供七种布局：横向三列、纵向三行、左右主视图、上下主视图及包含 3D 的四宫格。CT、MR、独立 PET MPR 和 CT 4D 支持全部七种布局；融合布局保持现状。切换保留定位、窗值、测量和标注。
