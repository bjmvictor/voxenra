...
MPR 右侧「MPR 布局」提供七种布局：横向三列、纵向三行、左右主视图、上下主视图及包含 3D 的四宫格。适用于 CT、独立 PET MPR 和 4D；融合布局保持现状。切换保留定位、窗值、测量和标注。

四宫格的 3D 复用当前时相体数据，以三色切面边框和青色交点圆环提供空间参考。边框颜色跟随十字线设置；它表示单个切面，不表示厚层投影的全部范围。可只显示交点或隐藏参考。拖动圆环在当前观察平面中移动 MPR 交点，限制在影像体范围内。3D 格顶部提供旋转、平移、缩放和重置，布局面板提供模板／PET 色表及六方向。
