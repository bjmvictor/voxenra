...
在 CT 或静态 PET 的 MPR 标签页、或 CT 4D 标签页选择「阈值分割」或「VOI」，启用后在轴位、冠状位、矢状位任一切面绘制。分割使用矩形柱体，VOI 使用圆形选区对应的球体/旋转椭球。深度表示沿绘制切面法线向两侧延伸的总厚度，单位 mm。开启工具会关闭厚层投影和手动配准，使绘制依据当前实际切面。
...
可绘制多个分割和 VOI，切换工具后保留已有范围。列表选择会切换到对应工具。底部原重置栏在分割工具下显示「清除分割 / 全部清除」，在 VOI 工具下显示「清除 VOI / 全部清除」；按钮不随内容滚动，没有对应范围时禁用。「全部清除」仅影响当前 MPR 标签页；4D 中仅清除当前时相的两种范围。底部操作行统一为 40 像素高，没有重置操作的工具不显示重置按钮，禁用的清除按钮保留按钮外观。
...
4D 的「当前时相播放」循环浏览当前时相的切片；「4D 播放」循环切换时相，两者互斥。绘制或编辑前先停止播放。

分割与 VOI 记录所属时相，统计使用该时相的原始体素。切换时相隐藏上一时相的范围，返回后恢复；后台过期统计不会覆盖新时相。工作区保存、恢复保留各时相的范围，清除仅影响当前时相。当前不提供跨时相自动传播、形变追踪或时序统计。
...
本版支持矩形柱体范围内阈值分割、球体/旋转椭球 VOI，以及下述 SEG 交换与自由形状单层转换。结果可随工作区保存。尚不提供画笔逐体素修改、跨层轮廓插值、连通域生长、分割三维表面或 RTSTRUCT 交换。PET MIP 是投影视图，不支持绘制和三维定量；请使用另外三个切面。
...
自动化验证使用合成 DICOM；本机 CT / MR 的 SEG 互操作记录见 [验证说明](validation/segmentation-import-20260916/README.md)。手册图片随应用打包，可离线查看。

## SEG 导入与管理

1. 左侧导入 DICOM 原影像，打开对应的 MPR 或 4D 标签页，4D 先选择时相。
2. 点击右侧工具栏独立的「导入」按钮，再选择「导入 SEG 分割」。右侧入口专用于当前影像的附属结果，不导入原始影像。
3. 成功后点击「分割管理」查看区域；可选择、改名、换色、显隐、删除和撤销重做。MR 的分割管理也可使用，但不会开放 CT/PET 阈值绘制。
4. 保存工作区可保留实际掩膜，恢复不依赖原 SEG 文件。右侧「导出」可再次输出 SEG / SR。

支持二值 SEG 的多个区域、重叠区域、稀疏帧和与原网格对齐的裁剪/翻转帧。校验患者标识、检查、参考坐标系、源 SOP 与增强多帧帧号；不匹配、来源不明确、重复帧或不支持的类型会整次拒绝，不留下部分区域。4D 掩膜按时相保存；导入过程中目标关闭、时相改变或取消，结果不会写入目标。

目前不对导入掩膜自动重采样；Fractional、LABELMAP 或不同采样间距的 SEG 不在支持范围。导入区域的几何固定，不能套用阈值或拖动包围框；体素数和体积包含所有掩膜体素，强度统计仅使用其中的有限源值。

## 自由形状 ROI 转为单层分割

在 MPR / 4D 的「测量」中画出并选中自由形状 ROI，点击「转为分割」。轮廓按原始体素中心采样，落到最接近当前切面的一个原始体素层，不向其他层延伸。支持平行于原始体素轴的三个切面及不同显示采样间距；暂不支持斜切或厚层投影轮廓。

转换保留原测量，产生可撤销、保存和导出为 SEG 的独立掩膜。之后修改原 ROI 不会自动更新掩膜；单层体积取决于原始体素尺寸，不能把它当作完整三维勾画的体积。
