...
右侧「导出」提供「导出 SEG」和「导出 SR 报告」。导出当前标签页的已完成结果；分割使用当前时相，每个区域单独一个二值 SEG 文件，因此重叠区域不会合并。SR 使用 TID 1500，包含测量数值、单位、空间位置与原影像引用；有体积区域时同时输出关联的 SEG。导入的区域再次导出时保留组织编码、算法描述和追踪标识，采用当前名称与颜色。
