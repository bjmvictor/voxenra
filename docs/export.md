...
3. 检查导出位置，点击“开始导出”。完成后点击高亮的结果路径，打开导出文件夹。

导出面板和整序列导出对话框以「查看导出说明」链接进入操作手册，格式差异和匿名范围在手册中说明。PNG 截图与测量 CSV／PDF 导出成功后，点击高亮路径可在 macOS Finder／Windows 文件资源管理器中选中文件；整序列 DICOM／PNG 则打开结果目录。长路径单行省略，悬停查看完整位置。
...
当前视图尚未加载成功、加载失败或平铺可见切片未全部就绪时，右侧 PNG 按钮禁用并说明原因，直接调用导出接口也会拒绝。源 DICOM 复制不依赖显示成功。左侧逐帧 PNG 使用独立转换流程（支持 RGB／调色板），是否可导出取决于源对象能否正确解码与转换，不能根据主阅片失败一概保证成功。
...
压缩像素采用与阅片相同的内置解码策略（RLE、JPEG、JPEG-LS、JPEG 2000）。缺少组件、未知编码或解码失败会显示对应原因，整次 PNG 导出失败并清理临时文件。
DICOM 导出不需要解压像素。
...
右侧导出面板提供测量 CSV 和 PDF，默认当前页签与匿名，可选择全部页签；PDF 可附当前切片参考图。只导出已完成的测量、标注和 VOI／分割统计。格式、单位、匿名处理及参考图范围见 [工作区与测量导出](workspaces.md#测量结果导出)。

## DICOM SEG / SR

右侧「导出」提供「导出 SEG」和「导出 SR 报告」。导出当前标签页的已完成结果；分割使用当前时相，同一源网格的区域合并为多区域二值 SEG，重叠部分分别保留；大数据按内存预算分批。SR 使用 TID 1500，每个 SEG 对应一份分割统计 SR，长度／角度／ROI 另存平面测量 SR；包含数值、单位、空间位置与原影像引用；有体积区域时同时输出关联的 SEG。导入的区域再次导出时保留组织编码、算法描述和追踪标识，采用当前名称与颜色。

这些结果保留患者身份及源影像引用，**不应用上方匿名化选项**。输出是未审核的初步结果；不会修改原始影像。尚不支持 SR 导入或 RTSTRUCT 交换。

合并审查已将多个独立 SEG 的共同 SR 改为多区域 SEG 与对应统计 SR，并分离平面测量，避免混合读取。Slicer 5.12.4 仍不能可靠加载自由形状等平面测量 SR；可用 PDF／CSV 查看数值。历史问题见 [交叉验证记录](validation/slicer-roundtrip-20260916/README.md)，本次结果见 [main 整合验收](validation/main-integration-20260916/README.md)。
