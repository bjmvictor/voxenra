...
- 常规CT/PET影像的2D视图， 支持调窗、翻页、平移、翻转、缩放、伪彩、测量导出等操作
- 常规CT/PET影像的平铺视图。支持调窗、伪彩、测量、翻页等操作
- 常规CT/PET影像的MPR视图，支持测量、十字线移动、十字线旋转、3D旋转、分割、VOI等操作
- 常规CT/PET影像的3D视图，支持旋转、缩放、平移、裁剪等
...
- 勾选两组影像序列，右键选择 **Compare 2D 对比**，直接打开左右对比页签。也可右键一组序列，在选择界面中选另一组；列表按患者优先排序，显示序列描述、患者、日期及切片数。
- 两侧共用一个翻页滑块，以左侧序列为基准（左侧为单张时使用右侧）按**相对进度**同步浏览，切片数量可以不同。相同进度不代表相同解剖位置，本功能不自动配准。
- **视口设置 → 对比同步操作**可以分别控制翻页、调窗、平移、缩放、旋转、镜像、伪彩、反白和显示信息，默认全部开启。关闭翻页同步后，滑块仅控制选中视口；重新开启某项同步时，以当前选中视口为准。
- 测量和标注各自独立，支持撤销重做。当前视图导出使用选中视口，测量报告包含两侧结果。工作区保存、恢复和独立窗口移动保留两侧状态与同步选项。
...
- **MPR 布局**：右侧可选横向三列、纵向三行、左右／上下主视图和含 3D 的四宫格，适用于 CT、独立 PET MPR 和 4D。3D 显示切面边框与交点，拖动交点同步定位；支持可选的双向旋转联动，默认独立观察。布局和 3D 显示状态随工作区保存。
