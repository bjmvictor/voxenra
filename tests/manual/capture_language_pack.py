...
QT_QPA_PLATFORM=cocoa QT_QUICK_BACKEND=software PYTHONPATH=src:tests \
  .venv/bin/python tests/manual/capture_language_pack.py docs/screenshots/35-language-pack.gif
...
def capture(destination):
    ...
    with TemporaryDirectory(prefix='voxenra-language-demo-') as temp:
        ...
        app, engine, window = launch(qt, settings_path)
        try:
            frames.append(shot(window, '1  设置 → 外观与语言'))
            ...
            frames.append(shot(window, '2  打开目录：内置语言已导出为可编辑副本'))
            ...
            frames.append(shot(window, '3  放入 es-ES.json，然后重启软件'))
        finally:
        ...
        try:
            ...
            QTest.mouseClick(window, Qt.LeftButton, Qt.NoModifier, point)
            pump(qt)
            frames.append(shot(window, '4  重启后，Español 自动出现在语言列表'))
            app.languageController.selectLanguage('es-ES')
            pump(qt)
            frames.append(shot(window, '5  选择新语言，即时生效'))
        finally:
            close(app, engine, window)
