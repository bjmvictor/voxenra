...
class WorkspaceDocumentController(QObject):
    ...
    @Property(str, notify=changed)
    def workspaceName(self): return Path(self._path).stem if self._path else "临时工作区"
    ...
    def recoveryStatusText(self):
        ...
        return {
            "saving": "正在更新自动恢复副本",
            "recoverable": "发现上次意外退出的恢复副本",
            "disabled": "自动恢复已关闭，仍可手动保存工作区",
            "error": "自动恢复副本保存失败：" + self._recovery_error,
            "idle": "自动恢复已启用，等待需要保存的内容",
            "pending": "更改等待自动保存（每 30 秒）",
            "saved": "自动恢复副本已更新 · " + self._last_recovery_at,
        }[state]
    ...
    def setAutomaticRecovery(self, enabled):
        ...
        if not saved:
            self._message, self._error = self.app.settingsController.message, True
            self.changed.emit()
        return saved
    ...
    def openRecoveryDirectory(self):
        if reveal_path(str(self._recovery_path.parent)):
            return True
        self._message, self._error = "恢复副本目录尚未创建或无法打开。首次自动保存后可打开该目录。", True
        self.changed.emit()
        return False
    ...
    def _capture(self, path):
        ...
        if any(getattr(view, "editBusy", False) for tab in tabs for view in tab.viewports_by_id.values()):
            raise ValueError("请等待裁剪或去床板完成后再保存工作区。")
    ...
    def save_to(self, path, *, recovery=False):
        ...
        try:
            ...
            self._message, self._error = str(error), True
            if recovery:
                self._recovery_error = str(error)
                self._message = "自动恢复副本保存失败：" + str(error)
            self.changed.emit()
            return False
        self._busy = True
        ...
        if not recovery:
            self._error = False
            self._message = "正在保存工作区…"
            self._dirty = False
        else:
            if self._message.startswith("自动恢复副本保存失败"):
    ...
    def _submit(self, function, kind):
        ...
        def done(result):
            try:
                ...
                payload = {"kind": kind, "error": str(error) or "操作未完成，请重试。"}
            if self._closed:
    ...
    def saveAs(self):
        if self._busy or self.panel.scanning:
            return False
        path, _ = QFileDialog.getSaveFileName(None, "保存工作区", self._path or "工作区.voxworkspace",
                                            "Voxenra 工作区 (*.voxworkspace)")
        if not path:
            return False
    ...
    def _confirm_replace(self, path):
        if not self._dirty:
            return True
        answer = QMessageBox.question(None, "打开工作区", "是否保存当前工作区的更改？",
    ...
    def open(self):
        if self._busy or self.panel.scanning:
            return
        path, _ = QFileDialog.getOpenFileName(None, "打开工作区", self._path,
                                            "Voxenra 工作区 (*.voxworkspace)")
        ...
        if "error" in payload:
            ...
            if payload["kind"] == "save":
                ...
                if recovery_save:
                    ...
                    self._message = "自动恢复副本保存失败：" + payload["error"]
                else:
                    self._dirty = True
            self._close_after_save = False
            self._open_after_save = None
            ...
            if not payload["recovery"]:
                self._message = "工作区已保存。"
                self._path = payload["path"]
                if self._dirty:
                    self._close_after_save = False
                    self._open_after_save = None
                    self._message = "已保存快照；保存期间产生的新更改仍待保存。"
            ...
            if payload["missing"]:
                self._busy = self._restoring = False
                self._message = f"{len(payload['missing'])} 个序列缺失或影像信息不一致。请重新定位源文件、文件夹或压缩包。"
                self._error = True
            else:
                self._begin_restore()
        self.changed.emit()
    ...
    def _advance_restore(self):
        ...
        try:
            if self._cancel.is_set():
                self._finish_restore("已取消后续恢复；已打开的页签保留。", error=True)
                return
            if self._loading_tab is not None:
                ...
                if state is not None and state.status == "error":
                    self._finish_restore("部分影像加载失败，可在对应页签重试。", error=True)
                    return
                if (state is not None and state.status != "ready") or tab._active_mpr_requests:
                    if time.monotonic() - started > 180:
                        raise ValueError("工作区恢复超时，请重试对应影像。")
                    return
                if is_source:
            ...
            if not self._load_queue:
                self._finish_restore("工作区已恢复。")
                return
            index, record = self._load_queue.pop(0)
            kind, uids = record["type"], record["series"]
            self._message = f"正在恢复页签 {index + 1} / {len(self._pending['document']['tabs'])}…"
            self.changed.emit()
            ...
            if tab is None:
                raise ValueError("无法恢复工作区页签。")
            self._loading_tab = (index, record, tab, time.monotonic(), fusion_volume, False)
        except Exception as error:
            self._finish_restore(f"恢复未完成：{error}", error=True)
    ...
    def _finish_restore(self, message, *, error=False):
        ...
        if payload is not None:
            ...
            if payload["missing"]:
                error = True
                message += f" 已跳过 {len(payload['missing'])} 个不可用序列；请另存工作区以保留原文件。"
    ...
    def requestClose(self):
        if self._closed or self._quit_approved:
            return True
        if self._busy:
            self._message = "正在保存或恢复工作区，请等待完成或取消恢复后再关闭。"
            self.changed.emit()
            return False
        if not self._dirty:
            self._quit_approved = True
            return True
        answer = QMessageBox.question(None, "保存工作区", "退出前是否保存当前工作区？",
        ...
        if answer == QMessageBox.Save:
            ...
            if not self.save():
                self._close_after_save = False
            return False
        self._quit_approved = answer == QMessageBox.Discard
