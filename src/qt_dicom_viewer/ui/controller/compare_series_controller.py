...
class CompareSeriesController(QObject):
    ...
    @Property(bool, notify=changed)
    def canConfirm(self):
        return self._anchor != self._partner and all(supports_compare(self._record(uid))
    ...
    def request(self, context_uid):
        ...
        if len(selected) == 2 and context_uid in selected and all(self.supportsSeries(uid) for uid in selected):
    ...
    def confirm(self):
        if self.canConfirm:
            self._open = False
            self.openRequested.emit(self._anchor, self._partner)
            self.changed.emit()
